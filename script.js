...
const cityDatabase = {
    ...
    "New York": {
        ...
        jobs: ["Finance", "Technology", "Media", "Healthcare"]
    },
    ...
    "Los Angeles": {
        ...
        jobs: ["Entertainment", "Technology", "Media", "Healthcare"]
    },
    ...
    "Miami": {
        ...
        jobs: ["Tourism", "Hospitality", "Finance", "Healthcare"]
    },
    ...
    "Austin": {
        ...
        jobs: ["Technology", "Software", "Business", "Healthcare"]
    },
    ...
    "Seattle": {
        ...
        jobs: ["Technology", "Software", "Engineering", "Healthcare"]
    },
    ...
    "Houston": {
        ...
        jobs: ["Energy", "Healthcare", "Technology", "Engineering"]
    },
    ...
    "Chicago": {
        ...
        jobs: ["Finance", "Manufacturing", "Technology", "Healthcare"]
    }
...
function displayCity(city, weather, wiki) {
    ...
    const dbInfo = cityDatabase[city.name];
    ...
    const populationValue = city.population || dbInfo?.population;
    const population = populationValue ? formatNumber(populationValue) : "Маълумот нест";
    ...
    searchResult.innerHTML = `
        <div class="city-result-card">
            <div class="city-result-image">
                ...
                <div class="city-badge">🇺🇸 ${escapeHTML(city.admin1 || dbInfo?.state || "USA")}</div>
            </div>
...
function showCityDetails(city, temperature, weatherText, feelsLike) {
    ...
    details.innerHTML = `
        <div class="detail-card">
            <span>📍</span>
            <h4>Ҷойгиршавӣ</h4>
            <p>${escapeHTML(city.name)}, ${escapeHTML(city.admin1 || info?.state || "")}, USA</p>
        </div>
...
function applyFilters() {
    ...
    const results = Object.entries(cityDatabase).filter(([name, city]) => {
        ...
        if (weather !== "all") {
            if (weather === "cold" && city.weather !== "cold" && city.weather !== "cool") return false;
            if (weather === "warm" && city.weather !== "warm") return false;
        }
    ...
    if (results.length === 0) {
        container.innerHTML = `
            <div class="search-error">
                ❌ Ягон шаҳр мувофиқ нест. Филтрҳоро тағйир деҳ.
            </div>
        `;
        return;
    }

    const weatherLabels = { warm: "Гарм", cool: "Муътадил", cold: "Сард" };
    const sizeLabels = { large: "Шаҳри калон", medium: "Шаҳри миёна" };
    ...
    container.innerHTML = results.map(([name, city]) => `
        <div class="filter-city-card">
            ...
            <p>
                📍 ${city.state} <br>
                🌤️ ${weatherLabels[city.weather] || city.weather} <br>
                👥 ${sizeLabels[city.size] || city.size}
            </p>
            <br>
...
function findMyCity() {
    ...
    Object.entries(cityDatabase).forEach(([name, city]) => {
        ...
        if (city.weather === weather) score += 4;

        const budgetGap = Math.abs(budgetOrder[city.budget] - budgetOrder[budget]);
        if (budgetGap === 0) score += 3;
        else if (budgetGap === 1) score += 1;

        if (city.goal.includes(goal)) score += 3;
...
// ==========================================
// 🏙️ POPULAR CITY CARDS (click + 3D tilt)
// ==========================================
...
function getFallbackImage(cityName) {
    ...
    const images = {
        ...
        "Chicago": "https://images.unsplash.com/photo-1494522358652-f30e61a60313?auto=format&fit=crop&w=1400&q=85",
        "Austin": "https://images.unsplash.com/photo-1531218150217-54595bc2b934?auto=format&fit=crop&w=1400&q=85",
        "Seattle": "https://images.unsplash.com/photo-1502175353174-a7a70e73b362?auto=format&fit=crop&w=1400&q=85"
    };
